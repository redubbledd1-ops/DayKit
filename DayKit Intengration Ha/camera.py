...
class PairingQrCamera(AgendaAlarmBackupEntity, Camera):
    """Toont de huidige koppelcode als QR-afbeelding - zelfde payload/beveiligingsmodel
    als sensor.agendaalarm_koppelcode's `qr_payload`-attribuut en
    /api/agendaalarm_backup/qr_code.png (zie pairing.py voor de uitleg van het
    ...
    def __init__(self, hub, entry: ConfigEntry) -> None:
        ...
        # de entity_picture-URL die het dashboard gebruikt) - AgendaAlarmBackupEntity
        # roept geen super().__init__() aan, dus dit moet hier expliciet.
        AgendaAlarmBackupEntity.__init__(self, hub, entry, ENTITY_KEY_PAIRING_QR_CAMERA, "Pairing QR")
