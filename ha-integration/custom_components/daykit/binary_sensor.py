...
class PhoneReachableBinarySensor(DayKitEntity, BinarySensorEntity):
    ...
    def _handle_update(self) -> None:
        """Hub-update binnengekomen (SIGNAL_UPDATE). Is het telefoon-IP daarbij veranderd - een
        verse koppeling die voor het eerst een adres meldt, of een nieuw adres uit de router -
        ping dan meteen in plaats van tot de volgende interval-tick te wachten.

        Zonder dit stond de sensor na de eerste koppeling nog tot PING_INTERVAL_SECONDS lang op
        "niet beschikbaar", en bleef hij na een IP-wijziging net zo lang het oude adres tonen.
        """
        if self.hub.last_known_phone_ip != self._pinged_ip:
            self.hass.async_create_task(self._async_ping(None))
            return
