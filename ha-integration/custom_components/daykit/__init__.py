"""DayKit - native dead-man's-switch backup alarm integration.
...
# Telemetrie die een HERSTART van Home Assistant moet overleven, op schijf in
# <config>/.storage/daykit.telemetry.
#
# Dit is bewust iets anders dan de _pending_rearm-truc verderop: die bewaart state alleen in
# het geheugen en dekt daarmee alleen een RELOAD van de integratie. Bij een echte herstart van
# HA begon de hub met last_known_phone_ip = None, en dan heeft de ping-sensor
# (binary_sensor.py) niets om te pingen - die staat dan "niet beschikbaar" totdat de app weer
# een alarm wapent. Dat kan uren duren (of nooit gebeuren als er even geen agenda-alarm
# aankomt), waardoor de sensor er kapot uitzag terwijl het IP allang bekend was geweest.
#
# Alleen telefoon-telemetrie, geen instellingen: die horen in de config entry options.
TELEMETRY_STORAGE_VERSION = 1
TELEMETRY_STORAGE_KEY = f"{DOMAIN}.telemetry"


def _telemetry_store(hass: HomeAssistant) -> Store:
    """Eén gedeelde Store voor alle config entries; per entry_id een eigen sleutel erin."""
    store = hass.data.setdefault(DOMAIN, {}).get("_telemetry_store")
    if store is None:
        store = Store(hass, TELEMETRY_STORAGE_VERSION, TELEMETRY_STORAGE_KEY)
        hass.data[DOMAIN]["_telemetry_store"] = store
    return store


async def _async_load_telemetry(hass: HomeAssistant, entry: ConfigEntry, hub) -> None:
    """Zet opgeslagen telefoon-telemetrie terug op de verse hub na een herstart van HA.

    Best-effort: een onleesbaar of ontbrekend bestand betekent gewoon "nog niets bekend",
    precies zoals vóór deze opslag bestond - het mag de setup nooit laten mislukken.
    """
    try:
        data = await _telemetry_store(hass).async_load() or {}
        stored = data.get(entry.entry_id) or {}
        if stored.get("last_known_phone_ip"):
            hub.last_known_phone_ip = stored["last_known_phone_ip"]
            _LOGGER.debug(
                "Telefoon-IP hersteld uit opslag: %s", hub.last_known_phone_ip
            )
    except Exception:  # noqa: BLE001
        _LOGGER.warning("Kon opgeslagen telefoon-telemetrie niet lezen", exc_info=True)


async def _async_save_telemetry(hass: HomeAssistant, entry: ConfigEntry, hub) -> None:
    """Schrijft de huidige telefoon-telemetrie weg. Aangeroepen zodra er een nieuw IP
    binnenkomt (arm/report_alive), niet periodiek - het IP wijzigt zelden."""
    try:
        store = _telemetry_store(hass)
        data = await store.async_load() or {}
        data[entry.entry_id] = {"last_known_phone_ip": hub.last_known_phone_ip}
        await store.async_save(data)
    except Exception:  # noqa: BLE001
        _LOGGER.warning("Kon telefoon-telemetrie niet opslaan", exc_info=True)
...
def _copy_default_sound_if_needed(hass: HomeAssistant) -> None:
    ...
    <config>/www/daykit_sounds/ zodat het publiek bereikbaar is via /local/... - zelfde map
    die http_views.py's DayKitSoundUploadView voor door de app geuploade geluiden gebruikt.
...
class AlarmBackupHub:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        # Spiegelt Android's ExternalSpeakerMode 1-op-1 (zie const.py) - alleen gebruikt om te
        # bepalen of _play_sound ook de telefoon via de HA Companion App laat meeklinken
        # (BOTH), zie _notify_phone_sound hieronder. Wordt vers ingelezen bij elke reload, en
        # een config-wijziging via /api/daykit/config triggert altijd zo'n reload (zie
        # _async_options_updated), dus dit loopt vanzelf in de pas met de app-instelling.
        self.speaker_mode: str = opts.get(CONF_SPEAKER_MODE) or DEFAULT_SPEAKER_MODE
        ...
        # POST /api/daykit/event (action=arm) - alleen onthouden/tonen
        ...
        # POST /api/daykit/event, ongeacht actie of of de verwerking slaagde
        ...
        # true = laat het volume van de fallback-speaker ongemoeid (zie _play_sound), gespiegeld
        # vanuit de app's HomeAssistantSettings.skipBackupVolume via config_sync.py.
        self.skip_volume: bool = bool(opts.get(CONF_SKIP_BACKUP_VOLUME, False))
    ...
    async def _async_set_phone_ip(self, phone_ip: str) -> None:
        """Onthoudt het laatst gerapporteerde telefoon-IP en schrijft het weg zodat het een
        herstart van HA overleeft (zie _async_save_telemetry).

        Alleen schrijven als het daadwerkelijk verandert: het IP blijft normaal maandenlang
        hetzelfde, en bij elke arm-aanroep naar schijf schrijven is zonde van de flash."""
        if phone_ip == self.last_known_phone_ip:
            return
        self.last_known_phone_ip = phone_ip
        await _async_save_telemetry(self.hass, self.entry, self)

    async def async_report_phone_ip(self, phone_ip: str) -> None:
        """Werkt alleen het telefoon-IP bij, zonder iets te wapenen of te stoppen (actie
        "report_ip", zie http_views.py).

        De app stuurt dit bij het koppelen, bij het openen van de HA-instellingen en bij elke
        periodieke sync-ronde. Daarmee heeft de ping-sensor meteen na de eerste koppeling een
        adres - vroeger moest daar eerst een agenda-alarm voor ingepland worden - en volgt hij
        een IP-wijziging door de router binnen één sync-ronde in plaats van pas bij het
        volgende alarm.
        """
        await self._async_set_phone_ip(phone_ip)
        self._notify_update()
    ...
    def generate_pairing_code(self) -> str:
        ...
        (sensor.daykit_pairing_code + QR-afbeelding). Overschrijft een eventuele
    ...
    def record_incoming_event(self, action: str, payload: dict) -> None:
        ...
        POST /api/daykit/event (sensor.py's LastEventSensor) - puur voor
        zichtbaarheid, geen eigen logica op deze velden. Aangeroepen door
        http_views.py's DayKitEventView.post ongeacht welke actie het is of of de
    ...
    async def _play_sound(self) -> None:
        ...
        # speaker_mode BOTH = HA-speaker + telefoon tegelijk (zie const.py's CONF_SPEAKER_MODE) -
        # dit geldt zowel voor de echte watchdog-fallback als voor de handmatige test via de
        # "Alarm active"-switch (switch.py), die dezelfde _play_sound-aanroep deelt zodat de
        # test realistisch is. Onafhankelijk van het HA-speaker-resultaat hierboven: best-effort,
        # nooit de reden dat de fallback als geheel faalt.
        if self.speaker_mode == "BOTH":
            await self._notify_phone_sound()

    async def _notify_phone_sound(self) -> None:
        """Laat de telefoon zelf ook geluid maken, via de officiële HA Companion App
        (notify_service) i.p.v. een aparte koppeling met de DayKit-app zelf - de
        Companion App ondersteunt een `tts_text`-payload die lokaal op het toestel wordt
        uitgesproken. Vereist dat notify_service naar diezelfde Companion App wijst (zelfde
        instelling als voor de start/stop-notificaties, zie _notify_start/_notify_stop)."""
        if not self.notify_service:
            _LOGGER.debug(
                "speaker_mode BOTH, maar geen notify_service geconfigureerd - "
                "telefoon-geluid overgeslagen"
            )
            return
        ...
        try:
            await self.hass.services.async_call(
                ...
                {
                    "message": "Backup alarm",
                    "data": {"tts_text": "Alarm! This is your backup alarm."},
                },
                blocking=False,
            )
        except Exception:  # noqa: BLE001
            _LOGGER.exception("Fout bij telefoon-tts (speaker_mode BOTH)")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Telefoon-IP van vóór de herstart terugzetten, vóór de platforms geladen worden - anders
    # doet binary_sensor.py's eerste ping (die meteen bij async_added_to_hass gebeurt) het nog
    # zonder IP en staat de sensor onnodig een hele interval lang op "niet beschikbaar".
    await _async_load_telemetry(hass, entry, hub)
    ...
    # instellingen pusht via /api/daykit/config) een lopende wapening/fallback
    ...
    # gebeurt, of via een POST /api/daykit/config vanuit de app (config_sync.py).
...
async def _restore_pending_rearm(hass: HomeAssistant, entry: ConfigEntry, hub: AlarmBackupHub) -> None:
    ...
    # update plaatsvond).
    # `or` i.p.v. .get(..., default): bij een reload staat deze sleutel altijd in pending, ook
    # als 'ie None is. Met een default zou een lege pending-waarde dus het IP overschrijven dat
    # _async_load_telemetry net van schijf had teruggezet.
    hub.last_known_phone_ip = pending.get("last_known_phone_ip") or hub.last_known_phone_ip
