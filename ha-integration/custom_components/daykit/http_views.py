...
class DayKitEventView(HomeAssistantView):
    ...
    body:
      ...
      {"action": "stop"}
      {"action": "disarm"}
      {"action": "report_ip", "phone_ip": "192.168.1.42"}
    ...
    "report_ip" bestaat omdat het IP eerder alleen meeliftte op "arm" en "start". Dat betekende
    dat de ping-sensor pas iets te pingen had zodra er een agenda-alarm ingepland stond - bij een
    verse koppeling, of bij iemand zonder aankomend alarm, bleef 'ie dus leeg. En als de router
    de telefoon een nieuw IP gaf, bleef de sensor het oude adres pingen tot het volgende alarm,
    met een onterechte "niet thuis" tot gevolg. Deze actie wapent niets en raakt geen enkele
    timer aan; ze werkt puur de telemetrie bij.
    """
    ...
    async def post(self, request: web.Request) -> web.Response:
        ...
        if action == "report_ip":
            phone_ip = data.get("phone_ip")
            if not phone_ip:
                return web.json_response({"error": "phone_ip is required"}, status=400)
            for hub in hubs:
                await hub.async_report_phone_ip(phone_ip)
            return web.json_response({"success": True, "phone_ip": phone_ip})
