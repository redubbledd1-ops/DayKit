"""Sensor entities voor DayKit (alleen-lezen zichtbaarheid van huidige fallback-config).
...
class SpeakerEntitySensor(DayKitEntity, SensorEntity):
...
class SoundUrlSensor(DayKitEntity, SensorEntity):
...
class PairingCodeSensor(DayKitEntity, SensorEntity):
...
class VolgendAlarmSensor(DayKitEntity, SensorEntity):
    """Toont het eerstvolgende geplande alarm-tijdstip, zoals meegegeven door de app via
    POST /api/daykit/event (action=arm) -> AlarmBackupHub.async_arm()."""
...
class LastEventSensor(DayKitEntity, SensorEntity):
    """Diagnose: toont wanneer/wat de laatste binnenkomende, geauthenticeerde aanroep op
    POST /api/daykit/event was - ongeacht of die actie slaagde (zie
...
class WatchdogStatusSensor(DayKitEntity, SensorEntity):
...
class OutOfBedSensor(DayKitEntity, SensorEntity):
...
class HomeLiveSensor(DayKitEntity, SensorEntity):
